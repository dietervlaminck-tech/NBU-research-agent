...
def _title(target):
    return target.get("title") or target.get("name") or ""


def _context(target):
    return target.get("research_question") or target.get("description") or "—"


def _analysis_context(analysis_id):
    """Returns (analysis, target, params) where target is the survey study or
    the dataset the analysis was run on."""
    analysis = db.get("analyses", analysis_id)
    if not analysis:
        raise ValueError("Analysis not found")
    if analysis.get("dataset_id"):
        target = db.get("datasets", analysis["dataset_id"]) or {}
    else:
        target = db.get("studies", analysis["study_id"]) or {}
    params = analysis.get("params") or {}
    return analysis, target, params


def _stata_script(analysis, study, params):
    kind = analysis["kind"]
    head = (f"* {analysis.get('title', kind)}\n"
            f"* Source: {_title(study)}\n"
